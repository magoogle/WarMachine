...
-- ---------------------------------------------------------------------------
-- Inject persisted state from core/persistence.lua.  Called on zone entry so
-- re-visiting an overworld region picks up where the last session left off.
--
-- visited_set  -- { [node_str] = true, ... }  cells we've walked before
-- blocked_set  -- { [node_str] = true, ... }  cells confirmed non-walkable
--                (skip the engine walk-check on next scan)
--
-- Walkable-but-not-visited cells aren't persisted; they'll be re-discovered
-- on the first scan after entry.
-- ---------------------------------------------------------------------------
explorer.import = function (visited_set, blocked_set)
    if visited_set then
        for k in pairs(visited_set) do
            if explorer.visited[k] == nil then
                explorer.visited[k] = k
                explorer.visited_count = explorer.visited_count + 1
                -- Visited cells are by definition scanned + walkable.  Mark
                -- as scanned=true so the next update() doesn't re-frontier
                -- them and waste a walk-check.
                explorer.scanned[k] = true
            end
        end
    end
    if blocked_set then
        for k in pairs(blocked_set) do
            -- Don't overwrite a true (walkable) entry with false (blocked) --
            -- if both sets contain k for any reason, walkable wins.
            if explorer.scanned[k] == nil then
                explorer.scanned[k] = false
            end
        end
    end
end
...
-- Persistent fail counter so a target that's failed many times graduates
-- from `retry` (transiently blocked, may be re-presented) to
-- `scanned[k]=false` (treated as a wall by pathfinder + persistence).
-- Without this, a stuck cell cycles indefinitely:
--   fail -> add_visited + add_retry -> next update() sees it in retry,
--   yanks it from visited, re-adds to frontier, bot re-picks it, fails again.
-- After FAIL_BLOCK_THRESHOLD failures the cell is hard-blocked; the
-- persistence layer (core/persistence.lua) writes blocked cells to disk
-- so the dead end survives across sessions for overworld zones (and
-- across crashes within a procedural run).
local FAIL_BLOCK_THRESHOLD = 3
local _fail_counts = {}

-- Cleared when the navigator resets (e.g., zone change).  Without this
-- the fail counts would carry over into a new zone where the same
-- node_str refers to a different cell.
explorer.clear_fail_counts = function ()
    _fail_counts = {}
end
...
explorer.select_node = function (local_player, failed)
    ...
    if failed ~= nil then
        ...
        local count = (_fail_counts[failed_str] or 0) + 1
        _fail_counts[failed_str] = count
        add_visited(failed_str)
        if count >= FAIL_BLOCK_THRESHOLD then
            -- Hard-block: scanned=false so pathfinder treats as wall AND
            -- persistence writes it to disk.  Don't add to retry -- we want
            -- this cell permanently excluded for this session (and further
            -- sessions in overworld zones via the persistence layer).
            explorer.scanned[failed_str] = false
            remove_retry(failed_str)
            -- Drop any frontier entry that might still be pointing at it.
            remove_frontier(failed_str)
            utils.log(1, 'fail-block: ' .. failed_str ..
                ' after ' .. tostring(count) .. ' failures')
        else
