...
-- `_G.WARMACHINE_TRAVEL_MODE` boolean.  UniversalRotation reads it in
-- its rotation_engine.lua spell loop and skips spells where
-- `cfg.self_cast == false`.  `self_cast` is the existing per-spell
-- checkbox in the rotation profile -- defensives are conventionally
-- self_cast in real builds, so no new tagging is required.
--
-- Usage from a task:
--
...
M.set_travel_mode = function (active)
    _G.WARMACHINE_TRAVEL_MODE = active and true or false
end

M.get_travel_mode = function ()
    return _G.WARMACHINE_TRAVEL_MODE == true
end
...
-- chosen to engage.  UniversalRotation reads `_G.WARMACHINE_TARGET` in
-- its spell loop and prefers it over its own target_selector pick when
-- present + valid -- so when WarMachine decides we're attacking the
-- Soulspire 30y away, UR casts at the Soulspire instead of the closer
-- mob the orbwalker is auto-targeting.  Without this, UR was firing
-- spells at whichever enemy the host's enemy stream put first, and
-- orbwalker's cursor was pulling the bot's facing back to those mobs --
-- the user-reported "keeps turning around to fight monsters" symptom
-- when WarMachine wants to engage a structure.
--
...
M.set_kill_target = function (actor)
    _G.WARMACHINE_TARGET = actor or nil
end

M.get_kill_target = function ()
    return _G.WARMACHINE_TARGET
end
...
-- outlive WarMachine.
M.clear = function ()
    _G.WARMACHINE_TRAVEL_MODE = false
