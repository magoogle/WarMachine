...
task.shouldExecute = function ()
    ...
    -- Detect manual button press (rising edge)
    if gui.elements.warplan_test_button:get() and not state.pending then
        state.pending      = true
        state.step         = STEP_VERIFY_MENU
        state.current_slot = 1
        state.timer        = get_time_since_inject()
        state.baseline     = #get_quests()
        state.result       = nil
        state.start_pos_x  = nil
        state.start_pos_y  = nil
        console.print(string.format('[WarMachine] Click test started — baseline quest count = %d', state.baseline))
    end
